...
fn multisig_deposit_is_taken_and_returned() {
	new_test_ext().execute_with(|| {
		...
		// deposit register multisig fee
		Balances::make_free_balance_be(&1, Balances::free_balance(1) + 3);  // register fee: 3
		...
		assert_eq!(Balances::free_balance(1), 2); // reserve amount: 3
...
fn cancel_multisig_returns_deposit() {
	new_test_ext().execute_with(|| {
		...
		// deposit register multisig fee
		Balances::make_free_balance_be(&1, Balances::free_balance(1) + 4);
...
fn timepoint_checking_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(3), multi, 5));
		// deposit register multisig fee
		Balances::make_free_balance_be(&1, Balances::free_balance(1) + 3);
		let call = call_transfer(6, 15);
